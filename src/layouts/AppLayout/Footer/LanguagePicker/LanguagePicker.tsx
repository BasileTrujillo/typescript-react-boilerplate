...
export const LanguagePicker = () => {
    ...
    const setLanguage = (lang: string) => appContext.dispatch({ type: 'change-language', lang });
    ...
    const selectedOptions = options.find((value: any) => value.key === appContext.state.lang);
        ...
        <div>
            ...
            >
                {options.map((value: any) => (
                    <MenuItem key={value.key} selected={value.key === appContext.state.lang} onClick={getLocalSelectHandler(value.key)}>
                        {value.label}
                    </MenuItem>
                ))}
            </Menu>
